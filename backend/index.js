        ...
        res.status(500).json({
            ...
            error:
                process.env.NODE_ENV === "development"
                    ? err.message
                    : undefined, // Jangan ekspos error detail pada produksi
        });
    }
});
...
app.post("/volunteer", async (req, res) => {
    try {
        ...
        if (
            !interest_reason ||
            typeof interest_reason !== "string" ||
            interest_reason.trim() === ""
        ) {
            return res.status(400).json({
                message:
                    "Interest reason is required and must be a valid string.",
            });
        }
        if (
            !suitability_reason ||
            typeof suitability_reason !== "string" ||
            suitability_reason.trim() === ""
        ) {
            return res.status(400).json({
                message:
                    "Suitability reason is required and must be a valid string.",
            });
        }
        if (
            !job_role ||
...
app.delete("/event/:id", async (req, res) => {
    ...
    try {
        const [result] = await db.execute("DELETE FROM event WHERE id = ?", [
            eventId,
        ]);
...
app.post("/report_issue", upload.single("photo"), async (req, res) => {
    try {
        const { full_name, phone, title, location, description, expectation } =
            req.body;
...
app.get("/issue/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.execute("SELECT * FROM issue WHERE id = ?", [
            id,
        ]);
        if (result.length === 0) {
...
app.patch("/issue/approval", async (req, res) => {
    ...
    if (!status_aproval || !id) {
        return res
            .status(400)
            .json({ error: "Permintaan aproval issue tidak valid" });
    }

    try {
        const [result] = await db.execute(
            "UPDATE issue SET approval_status = ? WHERE id = ?",
            [status_aproval, id]
        );
        ...
        const [updatedIssue] = await db.execute(
