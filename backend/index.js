...
app.post('/report_issue', upload.single('photo'), async (req, res) => {
    ...
    `;
        ...
        res.status(201).json({ message: 'Laporan berhasil disimpan', photo_url: `/uploads/${photo}` });
    } catch (error) {
        console.error('Error:', error);
...
app.put('/user', async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.execute('UPDATE user SET fullName = ?, email = ?, password = ? WHERE id = ?', [
            fullName,
            email,
            password,
            id,
